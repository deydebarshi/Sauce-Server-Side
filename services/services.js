...
const ApiUrl = async () => {
  ...
  if(search !== null){
    api_link = api_link + '&search=' + search;
  }
...
const autoCompleteAddress = async text => {
  ...
  params.append('url', 'https://maps.googleapis.com/maps/api/place/autocomplete/json?input=' + encodeURIComponent(text));
  params.append('key', 'AIzaSyCf4y8NJ92lQZMTiAgsx2c5Upt0i52Ft88');
  params.append('sessiontoken', generateRandomNumber);
  params.append('autocomplete', true);

   try {
     let Items = await axios.post(`https://thesauce.app/api/api.php`,
     params,
...
const geocodeFromAddress = async text => {
  ...
  const params = new URLSearchParams();
  params.append('url', 'https://maps.googleapis.com/maps/api/place/textsearch/json?query='+encodeURIComponent(text));
  params.append('key', 'AIzaSyCf4y8NJ92lQZMTiAgsx2c5Upt0i52Ft88');
  params.append('sessiontoken', generateRandomNumber);
  params.append('textsearch', true);
...
const getCuisinList = async () => {
  ...
  let api_link = 'https://api.thesauce.app/cuisines.json/'
...
const sendSignUpDetails = async (fname ,  email) => {
  try {
    let List = await axios.post('https://api.thesauce.app/subscribe/' , 
    {
      name  : fname,
      email : email
    },
...
module.exports.sendSignUpDetails = sendSignUpDetails;
